...
import {
  type CursorPayload, createReadImageResolver, encodeCursor,
  isValidCursorPayload, readTranscriptPage,
...
describe("readTranscriptPage", () => {
  let testDir: string;
  beforeEach(async () => { testDir = await mkdtemp(join(tmpdir(), "omp-trans-test-")); });
  ...
  it.each([
    ...
    const records = [{ type: "header", ver: 1 }, ...Array.from({ length: count }, (_, i) => ({
      type: "message", id: `m-${i}`, message: { role: "user", content: `Msg ${i}` },
    }))];
  ...
  it("supports arbitrarily long sessionId, bounding cursor <=512 and completing page 2", async () => {
    ...
    const records = Array.from({ length: 51 }, (_, i) => ({ type: "message", id: `m-${i}`, message: { role: "user", content: `M ${i}` } }));
    await writeFile(sessionPath, records.map((r) => JSON.stringify(r)).join("\n") + "\n");
    ...
    const cross = await readTranscriptPage({ sessionId: "other-session", sessionPath, cursor: p1.olderCursor });
  ...
  it("supports append-only file growth and invalidates truncation/replacement", async () => {
    ...
    const initial = Array.from({ length: 60 }, (_, i) => ({ type: "message", id: `msg-${i}`, message: { role: "user", content: `M ${i}` } }));
    ...
    await writeFile(sessionPath, JSON.stringify({ type: "message", id: "app-1", message: { role: "user", content: "A" } }) + "\n", { flag: "a" });
    ...
    expect((await readTranscriptPage({ sessionId: "s-app", sessionPath, cursor })).status).toBe("invalidated");
  ...
  it("authenticates cursor before path and handles ENOENT/ENOTDIR correctly", async () => {
    ...
    await writeFile(sessionPath, JSON.stringify({ type: "message", id: "m1", message: { role: "user", content: "hi" } }) + "\n");

    expect(await readTranscriptPage({ sessionId: "s-auth", sessionPath: null })).toEqual({ sessionId: "s-auth", messages: [], olderCursor: null, status: "unavailable" });
    expect(await readTranscriptPage({ sessionId: "s-auth", sessionPath: join(testDir, "missing.jsonl") })).toEqual({ sessionId: "s-auth", messages: [], olderCursor: null, status: "unavailable" });

    const validSignedCursor = encodeCursor({ s: hashSession("s-auth"), dev: 1, ino: 1, btime: 1, end: 100, next: 50 });
    for (const [p, s] of [[null, "s-auth"], [join(testDir, "deleted.jsonl"), "s-auth"], [sessionPath, "s-other"]] as const) {
      expect((await readTranscriptPage({ sessionId: s, sessionPath: p, cursor: validSignedCursor })).status).toBe("invalidated");
    }
  });
  ...
  it.each([
    ...
    ["unsafe int", { s: hashSession("s"), dev: Number.MAX_SAFE_INTEGER + 10, ino: 1, btime: 1, end: 10, next: 5 }],
  ...
  it("fails closed (invalidated) when hasOlder is true but stat identity is unsafe integer", async () => {
    ...
    await writeFile(sessionPath, Array.from({ length: 60 }, (_, i) => JSON.stringify({ type: "message", id: `m-${i}`, message: { role: "user", content: `M ${i}` } })).join("\n") + "\n");

    const res = await readTranscriptPage({
      sessionId: "s-unsafe", sessionPath,
      openFile: async (p) => {
  ...
  it("handles ToolCallTracker precedence, adjacent call-only/result, and >500-distance correlation", async () => {
    ...
    const records: Record<string, unknown>[] = [
      { type: "message", id: "older-alias-call", message: { role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: { path: "src/deep.ts" } }] } },
      ...Array.from({ length: 520 }, (_, i) => ({ type: "message", id: `mid-${i}`, message: { role: "user", content: `M ${i}` } })),
      {
        type: "message", id: "nearer-malformed-call",
        message: {
          role: "assistant",
          content: [
            { type: "text", id: "call_1", text: "colliding text" },
            { type: "toolCall", toolCallId: "", id: "call_1", toolName: "read", arguments: { path: "src/bad.ts" } },
            { type: "toolCall", id: "call_2", name: "read", arguments: { path: "src/adj.ts" } },
          ],
        },
      },
      { type: "message", id: "adj-res", message: { role: "toolResult", toolName: "read", toolCallId: "call_2", content: [{ type: "text", text: "Adj" }] } },
      { type: "message", id: "deep-res", message: { role: "toolResult", toolName: "read", toolCallId: "call_1", content: [{ type: "text", text: "Deep" }] } },
    ];
    await writeFile(sessionPath, records.map((r) => JSON.stringify(r)).join("\n") + "\n");
  ...
  it("truncates multi-megabyte text, invalidates on RangeError, and rethrows operational errors", async () => {
    ...
    await writeFile(sessionPath, JSON.stringify({ type: "message", id: "huge", message: { role: "assistant", content: "X".repeat(2 * 1024 * 1024) } }) + "\n");
    ...
    await expect(readTranscriptPage({
      sessionId: "s-lg", sessionPath,
      openFile: async () => { throw Object.assign(new Error("disk error"), { code: "EIO" }); },
    })).rejects.toThrow();
  });
  ...
  it("enforces image safety table and aggregate session budget via injected budget resolver", async () => {
    ...
    const imgRecords = [
      ...
      message: { role: "toolResult", toolName: "read", content: [{ type: "image", data: item.data, mimeType: item.mime }] },
    }));
    ...
    expect(imgMap.get("img-valid")).toEqual({ status: "available", mimeType: "image/png", data: validPng.toString("base64") });
